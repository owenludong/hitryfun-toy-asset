<template>
  <div class="index-page">
    ...
    <!-- Popular 独立显示：用 currentCategoryId === 550 判断 -->
    ...
    <!-- 非 Popular 类目：普通 div，依然使用 window 滚动 -->
...
/* 现在 scroll-view 只是一个普通容器，不再绝对定位 */
.scroll-view {
  background: #f8fafb;
}
...
.mobile-page-state-loading,
.mobile-page-state-default-loading,
.mobile-page-state-error,
.mobile-page-state-no-items {
  position: absolute;
  top: 50%;
  left: 50%;
  width: 200px;
...
.mobile-page-state-loading .page-state-loading-icon,
.mobile-page-state-default-loading .page-state-loading-icon,
.mobile-page-state-error .page-state-loading-icon,
.mobile-page-state-no-items .page-state-loading-icon {
...
// ⭐ 模块级变量：只在当前 JS 运行期内存里，不用 storage
let indexScrollY = 0
...
export default {
  ...
  watch: {
    // ⭐ 用 currentCategoryId 判断 Popular
  ...
  methods: {
    ...
    // 监听 window 的滚动，用于非 Popular 类目的无限加载
    handleWindowScroll () {
