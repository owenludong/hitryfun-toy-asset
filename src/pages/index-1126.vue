<template>
    <div class="index-page">
      ...
      <div class="popular-content" v-if="currentCategory === 'Popular'">
        <HeroCarousel />
        <!-- 🟢 全场包邮提示横条 -->
        <div class="cp-freeship-banner">🌎  {{ $t('freeShippingTips') }} ✨</div>
        <CouponsRow />
                ...
                <div class="mobile-feed-item" v-for="product in products" @click="showDetail(product.id)">
                  <div class="feed-card">
                    <div class="cp-img-wrapper">
                      <img class="feed-image" :src="product.mainImage || placeholderImg" :alt="product.name" />
                      <span v-if="[2460, 2461].includes(product.id)" class="cp-badge">Custom Link</span>
                    </div>
                    <h3 class="feed-title" :style="{display: '-webkit-box', WebkitLineClamp: 2,WebkitBoxOrient: 'vertical',overflow: 'hidden',textOverflow: 'ellipsis'}">{{ product.name || 'Custom Cat Squishy' }}</h3>
                    <p class="feed-price">
                      <span class="current-price">€{{ product.productPrice }}</span>
                      <span class="old-price" v-if="product.originalPrice">€{{ product.originalPrice }}</span>
                    </p>
                  </div>
                </div>
...
<style lang="scss" scoped>
  ...
  /* === ✅ 新版产品卡片样式（统一 Bestseller 风格） === */
  .mobile-feed-container-show {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(180px, 1fr));
    gap: 14px;
    padding: 14px 14px 14px 18px; /* ✅ 左边加宽4px */
    box-sizing: border-box;
 ...
 .feed-image {
  ...
  .cp-freeship-banner {
    background: linear-gradient(90deg, #a8e0c4, #9dd6c2);
    color: #ffffff;
    font-weight: 600;
    font-size: 0.9rem;
    text-align: center;
...
export default {
  ...
  data () {
    return {
      ...
      scrollOptions: {
        ...
        bounce: false   // ⭐ 新增：禁止上下拖出边界（不再拖到整屏空白）
      },
      ...
      categories: [
          {
            name: this.$t('categoryPopular'),
            id: 550
          },
          {
            name: this.$t('categoryCustom'),
            id: 603,
            showHot: true
          },
          {
            name: this.$t('categoryFat'),
            id: 600
          },
          {
            name: this.$t('categorySlim'),
            id: 601
          },
          {
            name: this.$t('categoryMini'),
            id: 602
          }
      ],
  ...
  methods: {
    ...
    searchByCategory (category, e) { // 目标位置定在距离左边120px
      ...
      // ⭐ 关键：切类目时，把 iScroll 位置重置到顶部
      this.$nextTick(() => {
        const iscroll = this.$refs.iscroll && this.$refs.iscroll.iscroll
        if (iscroll) {
          iscroll.scrollTo(0, 0, 0)   // 立即跳到顶部
          iscroll.refresh()
        }
      })
